...
/**
 * GerenteRegional controller.
 *
 * @Route("/gerenteregional")
 */
class GerenteRegionalController extends Controller {

    /**
     * @Route("/regiones_listar/{id_prod}/{matricula}", name="regiones_listar")
     * @Template()
     */
    public function listarAction(Request $request, $id_prod, $matricula) {
        ...
        foreach ($rResult as $alinea) {
            ...
            // Prospectos
            $linea[] = $em->getRepository('SistemaBundle:Prospectos')->getNroProspectosSup($matase);
            // Seleccionados
            $linea[] = number_format($em->getRepository('SistemaBundle:Prospectos')->getNroProspectadoSup($matase),0,'.',',');
            // Visitas
            $linea[] = $em->getRepository('SistemaBundle:Prospectos')->getNroVisitasSup($matase);
            // Cierres
            $linea[] = $em->getRepository('SistemaBundle:Prospectos')->getNroCierresSup($matase);
            // RAM
            $linea[] = number_format($em->getRepository('SistemaBundle:Prospectos')->getRamCierresSup($matase),0,'.',',');
            // Meta
            $linea[] = number_format(75000 * $em->getRepository('SistemaBundle:Usuario')->getNroAsesores($matase),0,'.',',');
            // Avance
            $linea[] = number_format(($linea[6] / $linea[7]) * 100, 2, '.', ',')." %";

            $linea[] = "<a href='".$this->generateUrl('supervisor_show', array('matricula'=>$linea[0]))."'><IMG src='../../public/images/icn_prospect.png' alt='Ver Prospecto' title='Detalle supervisor'></a>";
...
//        $matricula = $this->get('security.context')->getToken()->getUser()->getId();
     ...
     *
     * @Route("/supervisor_show/{matricula}", name="supervisor_show")
     * @Method("GET")
     * @Template()
     */
...
//        $matricula = $this->get('security.context')->getToken()->getUser()->getId();
        ...
        return $this->render("SistemaBundle:Reportes:index_supervisor.html.twig",array('matricula' => $matricula, 'asesor' => $nombrecompleto));
     ...
     *
     * @Route("/{id_prod}", name="gerenteregional")
     * @Method("GET")
     * @Template()
     */
    public function indexAction($id_prod) {
        ...
        return $this->render('SistemaBundle:Reportes:index_gerenteregional.html.twig', array('producto' => $id_prod, 'matricula' => $matricula,
